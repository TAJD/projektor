#!/usr/bin/env node
// Regression check for the DesignSystemConvention plugin (PROJ-527, Task 14
// scaffold): runs cofferdam against fixture.tsx and diffs its own findings
// (ignoring unrelated built-in findings, which vary across cofferdam
// versions) against expected.json. Exits non-zero and prints a diff on
// mismatch.
...
// fail-on threshold. Only a missing/malformed stdout is a real failure.
let raw;
try {
  raw = execFileSync(
    "npx",
