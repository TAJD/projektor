...
revisions. **KV** caches Access certs and email lookups. **R2** is bound for
...
service-layer contract), but a handful of deliberate REST↔MCP differences exist by
design - see [Contributor conventions → Deliberate REST↔MCP parity exceptions](/projektor/contributing/conventions/).
