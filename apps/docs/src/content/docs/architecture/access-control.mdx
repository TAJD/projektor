...
flowchart LR
    ...
    subgraph pk["Projektor — authorization"]
        provision["Provisioning:<br/>upsert user record"]
        member{"Auto-join configured?<br/>(default: no — invite-only)"}
        authz{"Group grants for<br/>the target project?"}
    end
    denied(["401 — never reaches Projektor"])
    noaccess(["403 — no workspace membership<br/>until an admin invites them"])
    pending(["Empty 'access pending' state"])
    scoped(["Sees only granted projects,<br/>at the granted role"])

    user --> gate
    gate -->|no| denied
    gate -->|yes| provision --> member
    member -->|no| noaccess
    member -->|yes, joins as viewer| authz
    authz -->|none| pending
    ...
    class gate gateCls;
    class provision,member,authz pkCls;
```

Access admits a first-time login and Projektor upserts a user record for them —
but by default that's it. Auto-join is invite-only unless an operator opts in
(`AUTO_JOIN_ROLE`, unset by default); a newly-admitted user with no workspace
membership gets **403 Forbidden** until an admin calls `invite_member`. Operators
who want an open instance can set `AUTO_JOIN_ROLE=viewer`, which auto-joins every
admitted user to the workspace as a `viewer` **with no groups**. Either way, groups
are never auto-granted: with no grants, the default-deny rule means a member sees
nothing — no projects, issues, or project wiki — until an admin adds them to a
group. This is the "access pending" state, and it is the whole point: membership
is not access.
...
100-bind-parameter ceiling on large workspaces.
